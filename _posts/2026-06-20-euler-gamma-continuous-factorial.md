---
title: "What Is (½)! ? — How Euler's Gamma Function Turned Factorials Into a Smooth Curve"
date: 2026-06-20
categories:
  - Special Functions
  - Mathematics
...
share: true
read_time: true
excerpt: "Everyone knows n! = 1·2·3⋯n. But what does (½)! mean? Euler asked this question in 1729 — and his answer, the Gamma function Γ(z), didn't just interpolate the factorial. It became the most important special function in mathematics, governing everything from the Gaussian integral to the volume of n-dimensional spheres. This is the story of how one interpolation problem gave birth to an analytic miracle."
---

**Challenge to the reader:** Using the Gamma function, compute $(1/2)!$ by hand. Then try $(-1/2)!$. The answers will tell you why $\sqrt{\pi}$ lives inside the factorial.
...
## 1. The Interpolation Problem Euler Was Solving
...
## 2. Euler's Construction and Modern Definition
...
### 2.1 Functional Equation via Integration by Parts
...
**Challenge to the reader:** Verify the integration by parts yourself. Set $u = t^z$ and $dv = e^{-t}dt$, then show the boundary term vanishes.

---

## 3. Why "Continuous Version" Is More Than Just a Slogan
...
### 3.2 Bohr–Mollerup Characterization
...
### 3.3 Analytic Extension to the Complex Plane
...
### 3.4 Preserves Factorial Structure
...
## 4. Euler's Role and Historical Context
...
## 5. Concrete Example: Non-Integer Factorial via Gamma
...
The Gamma function is not just a clever interpolation trick — it is the backbone of an enormous amount of modern mathematics and physics:

| Domain | Role of $\Gamma(z)$ |
|--------|---------------------|
| Probability | Gamma distribution, chi-squared distribution, Beta distribution |
| Number Theory | Functional equation of the Riemann zeta function |
| Combinatorics | Generalizing binomial coefficients to complex arguments |
| Physics | $n$-dimensional sphere volumes, Feynman diagrams, string theory |
| Complex Analysis | Meromorphic continuation, Weierstrass product, Stirling series |

The interpolation problem that Euler solved in 1729 turned out to be the thread that, when pulled, unraveled a vast connected web across mathematics.

**Final challenge:** The Bohr–Mollerup theorem says log-convexity makes $\Gamma$ unique. Try to construct a *different* function $g(x)$ on $(0,\infty)$ that satisfies $g(1)=1$ and $g(x+1)=xg(x)$ but whose logarithm is NOT convex. Plot it against $\Gamma(x)$ — where do they diverge, and why does the lack of log-convexity produce a "wobblier" interpolation?
