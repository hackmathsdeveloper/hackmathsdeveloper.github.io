---
title: "From 1/(1−z) to Elliptic Curves: 10 Problems That Hypergeometric Functions Secretly Solved"
date: 2026-08-11
categories:
  - Special Functions
  - Mathematics
...
share: true
read_time: true
excerpt: "What do the geometric series, arcsin, Legendre polynomials, Bessel functions, elliptic integrals, and the quintic equation all share? They are all hypergeometric series in disguise. Walk through 10 problems — from elementary to advanced — that reveal why the $${}_pF_q$$ family is the universal language of power series with rational coefficient ratios."
---

**Challenge to the reader:** Pick any three functions from the table below. For each one, write out the first four terms of its power series and verify by hand that the ratio of successive coefficients is a rational function of $$n$$. If it is, you've found a hypergeometric representation.
...
## 1. Elementary Series (Problems 1–4)
...
**Challenge to the reader:** Show that $$\arctan z = z\,{}_2F_1\!\left(\tfrac12,1;\tfrac32;-z^2\right)$$ by starting from the geometric series for $$1/(1+z^2)$$ and integrating termwise. This is the same pattern as Problem 4 — the parameters are just different.

---

## 2. Polynomial and ODE Structure (Problems 5–6)
...
## 3. Physics and Geometry (Problems 7–8)
...
## 4. Generalized Systems (Problems 9–10)
...
Why do these 10 problems all land in the same family? The answer is **rigidity**. A linear ODE with regular singular points on the Riemann sphere is determined up to isomorphism by its local monodromy data — and when that data satisfies a numerical condition (rigidity), the global equation is uniquely determined. Hypergeometric equations are precisely the rigid equations of rank 1 on $$\mathbb{P}^1\setminus\{0,1,\infty\}$$. Every one of the 10 problems above — from the geometric series to the Appell system — is a specialization of this rigidity principle. The $${}_pF_q$$ notation is not just a convenient shorthand; it is the **canonical coordinate** on the moduli space of rigid local systems.

---

**Final challenge to the reader:** Choose a function from your own field — the partition function of a statistical mechanics model, a Feynman integral, a generating function from combinatorics — and expand it as a power series. Compute $$c_{n+1}/c_n$$. If the ratio simplifies to a rational function of $$n$$, you have discovered a new hypergeometric representation. Write down its $${}_pF_q$$ parameters and predict its radius of convergence from the $$p,q$$ values. The table above is your template.
