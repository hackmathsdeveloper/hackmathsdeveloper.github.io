---
title: "Why You Can Add Points on a Curve — The Chord-and-Tangent Secret"
date: 2026-08-30
categories:
  - Elliptic Curves
  - Mathematics
...
share: true
read_time: true
excerpt: "The points of y² = x³ + ax + b form an abelian group, and the addition law is pure geometry: chords, tangents, and reflection — two formulas that work over every field. Part 2 of the five-part elliptic curves series."
---

**Challenge to the reader:** On $E:y^2=x^3-x$, compute $P+Q$ and $2P$ for $P=(0,0)$ and $Q=(-1,0)$. Watch out: you will need both the vertical-chord rule and the vertical-tangent rule.

*Part 2 of five in the series **Elliptic Curves & Elliptic Functions**: [Part 1 — The Circle, the Ellipse, and the Birth of a New Trigonometry]({% post_url 2026-08-30-ellipse-secret-elliptic-integrals %}) · [Part 2 — How to Add Points on a Curve]({% post_url 2026-08-30-chord-and-tangent-group-law %}) (this page) · [Part 3 — Counting Points, Keeping Secrets]({% post_url 2026-08-30-point-counting-hasse-cryptography %}) · [Part 4 — Rational Points and the Rank]({% post_url 2026-08-30-mordell-weil-rank-rational-points %}) · [Part 5 — The Torus, the $\wp$-Function, and Modularity]({% post_url 2026-08-30-torus-weierstrass-modularity %}). One figure — the two-panel derivation of the addition and doubling formulas — is reused throughout the series as a visual anchor.*

## The core theorem

The points of an elliptic curve, together with a point at infinity $\mathcal O$, form an **abelian group** — and the group law is geometric: draw a chord, find the third intersection, reflect. Two formulas compute everything, over *any* field.

**Why it matters.** This one geometric trick upgrades "solutions of a cubic" into "a group" — and the same two formulas that add points over $\mathbb R$ run elliptic-curve cryptography over finite fields (Part 3) and encode the rank of the rational points (Part 4). Everything downstream starts here.

---

## 1. The curve

Over a field, an elliptic curve is given (after a change of variables) by
$$E:\ y^2=x^3+ax+b,$$
...
The points of $E$, together with the point at infinity $\mathcal O$, form an **abelian group**, and the addition law is geometric — the *chord-and-tangent method* depicted in the figure:
...
## 3. Addition, following the left panel

The secant slope is $m=\frac{y_2-y_1}{x_2-x_1}$; the line is $y=m(x-x_1)+y_1$. Substituting into $y^2=x^3+ax+b$ and collecting terms yields a monic cubic in $x$ whose $x^2$-coefficient is $-m^2$, having exactly $x_1,x_2,x_3$ as roots. Vieta's formula (sum of roots $=-c_2$) gives
...
## 4. Doubling, following the right panel

Now the line is the tangent. Implicit differentiation of $y^2=x^3+ax+b$ gives $2y\,\frac{dy}{dx}=3x^2+a$, hence
...
The boxed formulas at the bottom of the figure:
...
**Challenge:** Re-derive $x_3=m^2-x_1-x_2$ from the expanded cubic — find the $x^2$ coefficient after substitution and apply Vieta's formula to the three roots $x_1,x_2,x_3$.

## 6. A worked example over $\mathbb Q$
...
## 7. The same figure over a finite field

Nothing in the figure's algebra required real numbers. Reduce $E:y^2=x^3+4x+4$ modulo $5$, and take $P_1=(1,3)$, $P_2=(0,2)$. Left panel, step by step, in $\mathbb F_5$:
$$m=\frac{2-3}{0-1}=\frac{-1}{-1}=1,\quad x_3=1-1-0=0,\quad y_3=1\cdot(1-0)-3=-2\equiv 3 .$$
So $P_1+P_2=(0,3)$ in $E(\mathbb F_5)$. [dummit.cos.northeastern](https://dummit.cos.northeastern.edu/docs/numthy_7_elliptic_curves.pdf) The picture of chords and reflections was drawn over $\mathbb R$, but the *formulas* are field-independent — a fact we exploit in Part 3, and whose deepest explanation arrives only in Part 5.

**Challenge:** On the same curve over $\mathbb F_5$, compute $2(1,2)$ with the doubling panel — you should land on $(2,0)$ — and conclude that $(1,2)$ has order 4.

---

## 8. The algebra behind the left panel, in full
...
The group law looks like a picture about real curves, but it is pure algebra: slopes are field operations, Vieta works over any field, and the discriminant condition is what keeps the tangent well-defined. That is why the same two formulas computed correctly over $\mathbb Q$, over $\mathbb F_5$, and over $\mathbb C$. The deepest explanation — that the chord-and-tangent law is literally addition on a torus — arrives in [Part 5]({% post_url 2026-08-30-torus-weierstrass-modularity %}).

---

**Final challenge:** Find all points of order 2 on $y^2=x^3-x$. (Hint: $2P=\mathcal O$ forces the tangent at $P$ to be vertical, so $y=0$.) Verify that the three of them, together with $\mathcal O$, form the Klein four-group under the chord-and-tangent law — each element is its own inverse, and the sum of any two gives the third.

*Next: [Part 3 — Counting Points, Keeping Secrets: Elliptic Curves mod $p$]({% post_url 2026-08-30-point-counting-hasse-cryptography %}), where the same two formulas become modular arithmetic and carry a cryptographic payload.*
